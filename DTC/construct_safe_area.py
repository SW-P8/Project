...
class ConstructSafeArea:
    ...
    def construct_safe_areas(route_skeleton: set, grid: dict, decrease_factor: float, find_relaxed_nn: bool = True) -> dict:
        cs = ConstructSafeArea._create_cover_sets(
            route_skeleton, grid, find_relaxed_nn)

        safe_areas = dict()

        for anchor in route_skeleton:
            safe_areas[anchor] = SafeArea.from_cover_set(
                cs[anchor], anchor, decrease_factor)
    ...
    def _create_cover_sets(route_skeleton: set, grid: dict, find_relaxed_nn: bool) -> dict:
        ...
        for split in sub_grid_keys:
            if split != []:
                ...
                sub_grid = CollectionUtils.get_sub_dict_from_subset_of_keys(
                    grid, split)
                task = mp.Process(target=ConstructSafeArea.create_cover_sets_sub_grid, args=(
                    route_skeleton, sub_grid, find_relaxed_nn, send_end))
                tasks.append(task)
    ...
    def create_cover_sets_sub_grid(route_skeleton: set, grid: dict, find_relaxed_nn: bool, send_end):
        ...
        if find_relaxed_nn:
            for (x, y) in grid.keys():
                candidates = ConstructSafeArea._find_candidate_nearest_neighbors(
                    route_skeleton_list, route_skeleton_kd_tree, (x + 0.5, y + 0.5))
                for point in grid[(x, y)]:
                    (anchor, dist) = DistanceCalculator.find_nearest_neighbor_from_candidates(
                        point, candidates, None)
                    cs[anchor].add((point, dist))
        else:
            for (x, y) in grid.keys():
                for point in grid[(x, y)]:
                    (anchor, dist) = DistanceCalculator.find_nearest_neighbour_from_candidates_with_kd_tree(
                        point, route_skeleton_list, route_skeleton_kd_tree)
                    cs[anchor].add((point, dist))
    ...
    def _find_candidate_nearest_neighbors(route_skeleton_list: list, route_skeleton_kd_tree: KDTree, cell: tuple) -> dict:
        ...
        candidate_indices = route_skeleton_kd_tree.query_ball_point(
            cell, min_dist + distance_to_corner_of_cell)
    ...
    def find_candidate_nearest_neighbors_with_historic_mindist(route_skeleton: set, cell: tuple) -> dict:
        ...
        for anchor in route_skeleton:
            print(anchor)
            dist = DistanceCalculator.calculate_euclidian_distance_between_cells(
                cell, anchor)
...
class SafeArea:
    def __init__(self, anchor: tuple[float, float], radius: float, cardinality: int, confidence_change, normalisation_factor, cardinality_squish, max_confidence_change) -> None:
        ...
        # Set as the fraction of a day 1 second represents. Done as TimeDelta is given in seconds.
    ...
    def calculate_radius(cover_set: set, decrease_factor: float):
        ...
        while removed_count < removal_threshold:
            ...
            filtered_cover_set = {(p, d)
                                  for (p, d) in filtered_cover_set if d <= radius}
    ...
    def get_current_confidence(self, timestamp: datetime) -> tuple[float, datetime]:
        ...
        new_confidence = self.confidence - \
            self.calculate_time_decay(delta.total_seconds())
    ...
    def update_confidence(self, dist, point: Point):
        ...
        if (distance_to_safearea <= 0):
            ...
            self.set_confidence(
                min(curr_conf + self.get_confidence_increase(), 1.0), time)
            self.cardinality += 1
        else:
            self.confidence -= self.calculate_confidence_decrease(
                distance_to_safearea)
    ...
    def calculate_time_decay(self, delta: float):
        ...
        # -0.5 forces the line to go through (0,0) and 2 normalizes the function such that it maps any number to a value between -1 and 1
        decay = self.sigmoid(decay, -cardinality_offset, -0.5, 2)
        decay = max(decay, 0.0)
    ...
    def get_confidence_increase(self):
        inc = max(min((1 / (self.cardinality * self.cardinality_squish)),
                  self.max_confidence_change), self.confidence_change_factor)
        return inc
