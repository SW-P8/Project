...
class NoiseCorrection:
    ...
    def correct_noisy_point(self, trajectory: Trajectory, point_id: int) -> None:
        ...
        grid_proj, geo_proj = self.setup_grid_to_geo_transformer(self.min_lat, self.min_lon)
        ...
        #new_point = self.convert_cell_to_point_2(self.initialization_point.get_coordinates(), nearest_anchor)
        logging.debug(f"Old point {trajectory.points[point_id].get_coordinates()}")
        trajectory.points[point_id].set_coordinates(new_point_switched)
