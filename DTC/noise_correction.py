...
class NoiseCorrection:
    ...
    def noise_detection(self, trajectory: Trajectory):
        ...
        for safearea in self.safe_areas.values():
            if safearea.confidence > 0.5:
                new_sa.update({safearea.anchor: safearea})
                new_sa[safearea.anchor].radius = 1000
        self.safe_areas = new_sa
