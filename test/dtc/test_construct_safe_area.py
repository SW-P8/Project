...
class TestSafeArea():
    ...
    def default_safe_area(self):
        cover_set = {(5, 5)}
        return SafeArea(cover_set, (5, 5), decrease_factor=0.1, confidence_change=0.01, cardinality_normalisation=100000, cardinality_squish=0.1, max_confidence_change=0.1)
    ...
    def test_calculate_timed_decay_do_not_change(self, default_safe_area):
        default_safe_area.cardinality = 10
        
        assert 0 == default_safe_area.calculate_timed_decay(0.1)


    def test_calculate_timed_decay_change(self, default_safe_area):
        default_safe_area.cardinality = 10
        default_safe_area.decay_factor = 0.1

        assert 0 < default_safe_area.calculate_timed_decay(1)

    def test_calculate_timed_decay_change_with_delta(self, default_safe_area):
        default_safe_area.cardinality = 10
        default_safe_area.decay_factor = 0.1
        
        decay_low = default_safe_area.calculate_timed_decay(0.1)
        decay_high = default_safe_area.calculate_timed_decay(1)
